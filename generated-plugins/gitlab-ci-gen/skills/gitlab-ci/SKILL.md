---
name: gitlab-ci
description: Generate GitLab CI pipelines
allowed-tools:
  - Read
  - Write
  - Edit
  - Bash
  - Grep
  - Glob
---

# gitlab-ci-gen

Generate GitLab CI pipelines.

## Tools Available

- **Read** — Read files from the project
- **Write** — Write new files to disk
- **Edit** — Edit existing files in place
- **Bash** — Run shell commands
- **Grep** — Search file contents with regex
- **Glob** — Find files by pattern

## Usage

Invoke the `gitlab-ci` skill to Generate GitLab CI pipelines. The skill analyzes your project structure and generates the appropriate code and configuration files.
