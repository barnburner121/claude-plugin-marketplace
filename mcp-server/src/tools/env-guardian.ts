...
export function registerEnvGuardianTools(server: McpServer) {
            ...
            text: JSON.stringify(
              {
                ...
                instructions:
                  "Use Grep tool to search for each pattern in the directory. Report any matches found with file path, line number, and which pattern matched. Suggest moving each to .env and adding to .gitignore.",
              },
              null,
              2
            ),
            ...
            text: JSON.stringify(
              {
                ...
                instructions:
                  "Search for each pattern to find all environment variables referenced in the codebase. Group them by category (database, API keys, app config, etc.). Generate a .env.example file with descriptive comments, placeholder values, and required/optional markers.",
              },
              null,
              2
            ),
            ...
            text: JSON.stringify(
              {
                ...
                example_path,
                instructions:
                  "Read both files. Compare variable names. Report: (1) Variables in .env.example but missing from .env, (2) Variables in .env but not in .env.example (potentially unused), (3) Variables with empty values that look required. Output a validation report.",
              },
              null,
              2
            ),
