...
export function registerCliTestingTools(server: McpServer) {
  server.tool("clitest_generate_suite", "Generate CLI integration tests", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "clitest_generate_suite", directory, instructions: "Generate CLI integration tests in the specified directory." }, null, 2) }] };
  });
  server.tool("clitest_add_snapshots", "Add CLI output snapshot tests", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    const { allowed, tier } = checkRateLimit(api_key);
    if (!allowed) return rateLimitError(tier);
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "clitest_add_snapshots", directory, instructions: "Add CLI output snapshot tests in the specified directory." }, null, 2) }] };
  });
  server.tool("clitest_add_mocks", "Generate CLI mock utilities (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "clitest_add_mocks", directory, instructions: "Generate CLI mock utilities in the specified directory." }, null, 2) }] };
  });
}
