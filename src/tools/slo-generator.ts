...
export function registerSloGeneratorTools(server: McpServer) {
  server.tool("slo_define_objectives", "Generate SLO/SLI definitions", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "slo_define_objectives", directory, instructions: "Generate SLO/SLI definitions in the specified directory." }, null, 2) }] };
  });
  server.tool("slo_calculate_budget", "Calculate error budget", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    const { allowed, tier } = checkRateLimit(api_key);
    if (!allowed) return rateLimitError(tier);
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "slo_calculate_budget", directory, instructions: "Calculate error budget in the specified directory." }, null, 2) }] };
  });
  server.tool("slo_generate_alerts", "Generate SLO-based alerts (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "slo_generate_alerts", directory, instructions: "Generate SLO-based alerts in the specified directory." }, null, 2) }] };
  });
}
