...
export function registerProductCatalogTools(server: McpServer) {
  server.tool("catalog_generate_schema", "Generate product catalog data model", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "catalog_generate_schema", directory, instructions: "Generate product catalog data model in the specified directory." }, null, 2) }] };
  });
  server.tool("catalog_add_categories", "Add category and taxonomy system", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    const { allowed, tier } = checkRateLimit(api_key);
    if (!allowed) return rateLimitError(tier);
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "catalog_add_categories", directory, instructions: "Add category and taxonomy system in the specified directory." }, null, 2) }] };
  });
  server.tool("catalog_add_search", "Generate product search and filtering (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "catalog_add_search", directory, instructions: "Generate product search and filtering in the specified directory." }, null, 2) }] };
  });
}
