...
export function registerSklearnPipelineTools(server: McpServer) {
  server.tool("sklearn_generate_pipeline", "Generate scikit-learn ML pipeline", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "sklearn_generate_pipeline", directory, instructions: "Generate scikit-learn ML pipeline in the specified directory." }, null, 2) }] };
  });
  server.tool("sklearn_add_preprocessing", "Add feature preprocessing steps", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    const { allowed, tier } = checkRateLimit(api_key);
    if (!allowed) return rateLimitError(tier);
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "sklearn_add_preprocessing", directory, instructions: "Add feature preprocessing steps in the specified directory." }, null, 2) }] };
  });
  server.tool("sklearn_add_evaluation", "Generate model evaluation framework (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "sklearn_add_evaluation", directory, instructions: "Generate model evaluation framework in the specified directory." }, null, 2) }] };
  });
}
