...
export function registerWebsocketSetupTools(server: McpServer) {
  server.tool(
    "ws_generate_server",
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                { action: "upgrade_required", instructions: "The ws_add_rooms tool requires a Pro subscription. Please upgrade to access WebSocket room management." },
                null,
                2
              ),
            },
