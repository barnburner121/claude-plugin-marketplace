...
export function registerDistributedTracingTools(server: McpServer) {
  server.tool(
    "trace_setup_otel",
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                { action: "upgrade_required", instructions: "The trace_configure_export tool requires a Pro subscription. Please upgrade to access trace export configuration features." },
                null,
                2
              ),
            },
