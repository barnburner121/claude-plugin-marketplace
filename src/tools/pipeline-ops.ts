...
export function registerPipelineOpsTools(server: McpServer) {
  ...
  server.tool(
    ...
    "Analyze CI/CD pipeline logs to diagnose build failures and suggest fixes",
    {
      log_content: z
        .string()
        .describe(
          "Pipeline log output (paste the failing log) or path to log file"
        ),
      platform: z
        .enum([
          "github-actions",
            ...
            text: JSON.stringify(
              {
                ...
                log_content:
                  log_content.length > 5000
                    ? log_content.slice(-5000)
                    : log_content,
                platform,
                instructions:
                  "Analyze the pipeline log for: (1) The root cause of failure (not just the symptom), (2) Whether it's a code issue, dependency issue, environment issue, or infra issue, (3) Specific fix with code/config changes needed, (4) Whether this is a flaky test or consistent failure, (5) Similar past issues and their solutions. Provide a clear diagnosis and step-by-step fix.",
              },
              null,
              2
            ),
