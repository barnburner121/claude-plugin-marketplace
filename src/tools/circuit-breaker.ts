...
export function registerCircuitBreakerTools(server: McpServer) {
  server.tool(
    "cb_generate_pattern",
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                { action: "upgrade_required", instructions: "The cb_configure_thresholds tool requires a Pro subscription. Please upgrade to access circuit breaker threshold tuning features." },
                null,
                2
              ),
            },
