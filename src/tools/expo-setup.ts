...
export function registerExpoSetupTools(server: McpServer) {
  server.tool("expo_generate_project", "Generate Expo managed project", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "expo_generate_project", directory, instructions: "Generate Expo managed project in the specified directory." }, null, 2) }] };
  });
  server.tool("expo_add_config", "Add Expo configuration plugins", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    const { allowed, tier } = checkRateLimit(api_key);
    if (!allowed) return rateLimitError(tier);
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "expo_add_config", directory, instructions: "Add Expo configuration plugins in the specified directory." }, null, 2) }] };
  });
  server.tool("expo_setup_eas", "Set up EAS Build and Submit (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "expo_setup_eas", directory, instructions: "Set up EAS Build and Submit in the specified directory." }, null, 2) }] };
  });
}
