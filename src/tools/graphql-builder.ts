...
export function registerGraphqlBuilderTools(server: McpServer) {
  server.tool(
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                { action: "upgrade_required", instructions: "The graphql_optimize_queries tool requires a Pro subscription. Please upgrade to access GraphQL query optimization." },
                null,
                2
              ),
            },
