...
export function registerWorkspaceSetupTools(server: McpServer) {
  server.tool(
    "workspace_generate_monorepo",
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                { action: "upgrade_required", instructions: "The workspace_add_packages tool requires a Pro subscription. Please upgrade to access package creation features in monorepo workspaces." },
                null,
                2
              ),
            },
