...
export function registerReactNativeSetupTools(server: McpServer) {
  server.tool("rn_generate_project", "Generate React Native project structure", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "rn_generate_project", directory, instructions: "Generate React Native project structure in the specified directory." }, null, 2) }] };
  });
  server.tool("rn_add_navigation", "Add React Navigation setup", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    const { allowed, tier } = checkRateLimit(api_key);
    if (!allowed) return rateLimitError(tier);
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "rn_add_navigation", directory, instructions: "Add React Navigation setup in the specified directory." }, null, 2) }] };
  });
  server.tool("rn_add_state", "Generate state management for React Native (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "rn_add_state", directory, instructions: "Generate state management for React Native in the specified directory." }, null, 2) }] };
  });
}
