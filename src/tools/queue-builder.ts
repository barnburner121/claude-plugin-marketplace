...
export function registerQueueBuilderTools(server: McpServer) {
  server.tool(
    "queue_setup_bullmq",
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                { action: "upgrade_required", instructions: "The queue_add_retry_logic tool requires a Pro subscription. Please upgrade to access advanced retry logic." },
                null,
                2
              ),
            },
