...
export function registerHelmChartTools(server: McpServer) {
  server.tool(
    "helm_generate_chart",
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                { action: "upgrade_required", instructions: "The helm_lint tool requires a Pro subscription. Please upgrade to access Helm linting and validation features." },
                null,
                2
              ),
            },
