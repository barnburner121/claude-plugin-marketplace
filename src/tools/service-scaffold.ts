...
export function registerServiceScaffoldTools(server: McpServer) {
  server.tool(
          ...
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(
                { action: "upgrade_required", instructions: "The service_add_healthcheck tool requires a Pro subscription. Please upgrade to access health check generation features." },
                null,
                2
              ),
            },
