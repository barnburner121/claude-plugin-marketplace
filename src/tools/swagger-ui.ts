...
export function registerSwaggerUiTools(server: McpServer) {
  server.tool("swagger_setup", "Generate Swagger UI setup", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "swagger_setup", directory, instructions: "Generate Swagger UI setup in the specified directory." }, null, 2) }] };
  });
  server.tool("swagger_customize", "Customize Swagger UI theme", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    const { allowed, tier } = checkRateLimit(api_key);
    if (!allowed) return rateLimitError(tier);
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "swagger_customize", directory, instructions: "Customize Swagger UI theme in the specified directory." }, null, 2) }] };
  });
  server.tool("swagger_add_auth", "Add authentication to Swagger UI (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ action: "swagger_add_auth", directory, instructions: "Add authentication to Swagger UI in the specified directory." }, null, 2) }] };
  });
}
