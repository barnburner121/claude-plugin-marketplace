...
export function registerMonitoringStackTools(server) {
    server.tool("monitor_setup_prometheus", "Generate Prometheus configuration with scrape targets and rules", {
    ...
    server.tool("monitor_setup_grafana", "Generate Grafana dashboard JSON with panels and data sources", {
    ...
    server.tool("monitor_create_alerts", "Create alerting rules for Prometheus Alertmanager (Pro)", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The monitor_create_alerts tool requires a Pro subscription. Please upgrade to access alert rule creation features." }, null, 2),
                    },
                ],
            };
        }
