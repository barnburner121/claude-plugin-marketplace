...
export function registerPostmortemGenTools(server) {
    server.tool("postmortem_create_template", "Generate post-incident review template", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "postmortem_create_template", directory, instructions: "Generate post-incident review template in the specified directory." }, null, 2) }] };
    });
    server.tool("postmortem_analyze_timeline", "Generate incident timeline analysis", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "postmortem_analyze_timeline", directory, instructions: "Generate incident timeline analysis in the specified directory." }, null, 2) }] };
    });
    server.tool("postmortem_track_actions", "Generate action item tracking (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "postmortem_track_actions", directory, instructions: "Generate action item tracking in the specified directory." }, null, 2) }] };
    });
}
