...
export function registerDockerComposeGenTools(server) {
    server.tool("compose_generate", "Generate a Docker Compose file with service definitions and networking", {
    ...
    server.tool("compose_add_service", "Add a new service definition to an existing Docker Compose file", {
    ...
    server.tool("compose_optimize", "Optimize a Docker Compose file for production readiness (Pro)", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The compose_optimize tool requires a Pro subscription. Please upgrade to access Docker Compose optimization features." }, null, 2),
                    },
                ],
            };
        }
