...
export function registerBlogEngineTools(server) {
    server.tool("blog_generate_setup", "Generate blog engine with MDX", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "blog_generate_setup", directory, instructions: "Generate blog engine with MDX in the specified directory." }, null, 2) }] };
    });
    server.tool("blog_add_rss", "Add RSS feed generation", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "blog_add_rss", directory, instructions: "Add RSS feed generation in the specified directory." }, null, 2) }] };
    });
    server.tool("blog_add_seo", "Generate blog SEO optimization (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "blog_add_seo", directory, instructions: "Generate blog SEO optimization in the specified directory." }, null, 2) }] };
    });
}
