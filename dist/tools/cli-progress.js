...
export function registerCliProgressTools(server) {
    server.tool("cliprogress_generate_bars", "Generate CLI progress bars", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "cliprogress_generate_bars", directory, instructions: "Generate CLI progress bars in the specified directory." }, null, 2) }] };
    });
    server.tool("cliprogress_add_spinners", "Add loading spinners", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "cliprogress_add_spinners", directory, instructions: "Add loading spinners in the specified directory." }, null, 2) }] };
    });
    server.tool("cliprogress_add_multibar", "Generate multi-progress display (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "cliprogress_add_multibar", directory, instructions: "Generate multi-progress display in the specified directory." }, null, 2) }] };
    });
}
