...
export function registerLazyLoadingTools(server) {
    server.tool("lazy_generate_patterns", "Generate lazy loading patterns", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "lazy_generate_patterns", directory, instructions: "Generate lazy loading patterns in the specified directory." }, null, 2) }] };
    });
    server.tool("lazy_add_intersection", "Add intersection observer loading", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "lazy_add_intersection", directory, instructions: "Add intersection observer loading in the specified directory." }, null, 2) }] };
    });
    server.tool("lazy_optimize_priority", "Optimize loading priorities (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "lazy_optimize_priority", directory, instructions: "Optimize loading priorities in the specified directory." }, null, 2) }] };
    });
}
