...
export function registerGithubActionsGenTools(server) {
    server.tool("gha_generate_workflow", "Generate a GitHub Actions workflow YAML with triggers and jobs", {
    ...
    server.tool("gha_add_caching", "Add caching steps to a GitHub Actions workflow for faster builds", {
    ...
    server.tool("gha_add_matrix", "Add matrix strategy to a GitHub Actions workflow for multi-version testing", {
    ...
    server.tool("gha_optimize", "Optimize a GitHub Actions workflow for speed and cost reduction (Pro)", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The gha_optimize tool requires a Pro subscription. Please upgrade to access GitHub Actions workflow optimization features." }, null, 2),
                    },
                ],
            };
        }
