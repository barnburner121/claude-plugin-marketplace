...
export function registerGeofencingTools(server) {
    server.tool("geofence_generate_system", "Generate geofencing implementation", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "geofence_generate_system", directory, instructions: "Generate geofencing implementation in the specified directory." }, null, 2) }] };
    });
    server.tool("geofence_add_triggers", "Add geofence trigger events", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "geofence_add_triggers", directory, instructions: "Add geofence trigger events in the specified directory." }, null, 2) }] };
    });
    server.tool("geofence_add_monitoring", "Generate geofence monitoring (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "geofence_add_monitoring", directory, instructions: "Generate geofence monitoring in the specified directory." }, null, 2) }] };
    });
}
