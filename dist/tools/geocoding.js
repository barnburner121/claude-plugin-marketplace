...
export function registerGeocodingTools(server) {
    server.tool("geo_setup_provider", "Set up geocoding provider integration", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "geo_setup_provider", directory, instructions: "Set up geocoding provider integration in the specified directory." }, null, 2) }] };
    });
    server.tool("geo_add_reverse", "Add reverse geocoding", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "geo_add_reverse", directory, instructions: "Add reverse geocoding in the specified directory." }, null, 2) }] };
    });
    server.tool("geo_add_batch", "Generate batch geocoding pipeline (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "geo_add_batch", directory, instructions: "Generate batch geocoding pipeline in the specified directory." }, null, 2) }] };
    });
}
