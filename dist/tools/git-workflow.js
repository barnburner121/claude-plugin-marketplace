...
export function registerGitWorkflowTools(server) {
    server.tool("git_generate_branching", "Generate a Git branching strategy configuration and documentation", {
    ...
    server.tool("git_setup_hooks", "Set up Git hooks for code quality enforcement", {
    ...
    server.tool("git_generate_templates", "Generate Git templates for PRs, issues, and commit conventions (Pro)", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The git_generate_templates tool requires a Pro subscription. Please upgrade to access Git template generation features." }, null, 2),
                    },
                ],
            };
        }
