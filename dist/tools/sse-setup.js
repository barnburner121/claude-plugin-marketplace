...
export function registerSseSetupTools(server) {
    server.tool("sse_generate_server", "Generate Server-Sent Events server", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "sse_generate_server", directory, instructions: "Generate Server-Sent Events server in the specified directory." }, null, 2) }] };
    });
    server.tool("sse_generate_client", "Generate SSE client with reconnection", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "sse_generate_client", directory, instructions: "Generate SSE client with reconnection in the specified directory." }, null, 2) }] };
    });
    server.tool("sse_add_auth", "Add authentication to SSE streams (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "sse_add_auth", directory, instructions: "Add authentication to SSE streams in the specified directory." }, null, 2) }] };
    });
}
