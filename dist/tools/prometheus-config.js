...
export function registerPrometheusConfigTools(server) {
    server.tool("prom_generate_config", "Generate Prometheus scrape config", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "prom_generate_config", directory, instructions: "Generate Prometheus scrape config in the specified directory." }, null, 2) }] };
    });
    server.tool("prom_add_rules", "Add recording and alerting rules", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "prom_add_rules", directory, instructions: "Add recording and alerting rules in the specified directory." }, null, 2) }] };
    });
    server.tool("prom_generate_dashboards", "Generate Grafana dashboards from metrics (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "prom_generate_dashboards", directory, instructions: "Generate Grafana dashboards from metrics in the specified directory." }, null, 2) }] };
    });
}
