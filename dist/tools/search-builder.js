...
export function registerSearchBuilderTools(server) {
    server.tool("search_setup_engine", "Set up a search engine integration with Elasticsearch, Meilisearch, or Typesense", {
    ...
    server.tool("search_generate_index", "Generate index configurations with field mappings, analyzers, and tokenizers", {
    ...
    server.tool("search_optimize_queries", "Pro: Optimize search queries with relevance tuning, faceting, and performance analysis", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The search_optimize_queries tool requires a Pro subscription. Please upgrade to access search query optimization." }, null, 2),
                    },
                ],
            };
        }
