...
export function registerTerraformModuleTools(server) {
    server.tool("tf_generate_module", "Generate a Terraform module with main, outputs, and provider configuration", {
    ...
    server.tool("tf_generate_variables", "Generate Terraform variable definitions with validation and descriptions", {
    ...
    server.tool("tf_best_practices", "Analyze Terraform code for best practices and compliance (Pro)", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The tf_best_practices tool requires a Pro subscription. Please upgrade to access Terraform best practices analysis." }, null, 2),
                    },
                ],
            };
        }
