...
export function registerLogSleuthTools(server) {
    // Tool 1: Analyze log files for patterns and anomalies
    server.tool("log_analyze", "Analyze log files for error patterns, anomalies, and trends", {
    ...
    server.tool("log_search", "Search through log files with structured queries — filter by level, time, message pattern, and context", {
        log_path: z.string().describe("Path to log file or directory"),
    ...
    server.tool("log_recommend", "Analyze codebase and recommend where to add structured logging for better observability", {
    ...
    server.tool("log_correlate", "Correlate log entries across multiple services using request IDs or timestamps (Pro feature)", {
        log_paths: z
            .array(z.string())
