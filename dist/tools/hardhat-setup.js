...
export function registerHardhatSetupTools(server) {
    server.tool("hardhat_generate_project", "Generate Hardhat development environment", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "hardhat_generate_project", directory, instructions: "Generate Hardhat development environment in the specified directory." }, null, 2) }] };
    });
    server.tool("hardhat_add_tests", "Add smart contract test setup", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "hardhat_add_tests", directory, instructions: "Add smart contract test setup in the specified directory." }, null, 2) }] };
    });
    server.tool("hardhat_add_deployment", "Generate deployment scripts (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "hardhat_add_deployment", directory, instructions: "Generate deployment scripts in the specified directory." }, null, 2) }] };
    });
}
