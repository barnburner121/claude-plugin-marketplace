...
export function registerProjectInitTools(server) {
    server.tool("init_generate_scaffold", "Generate a complete project scaffold with directory structure and configuration", {
    ...
    server.tool("init_add_tooling", "Add development tooling to a project (linting, formatting, testing)", {
        project_name: z.string().describe("Name of the project"),
    ...
    server.tool("init_configure_ci", "Configure CI/CD pipeline for the project with automated checks (Pro)", {
        project_name: z.string().describe("Name of the project"),
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The init_configure_ci tool requires a Pro subscription. Please upgrade to access CI/CD configuration features." }, null, 2),
                    },
                ],
            };
        }
