...
export function registerCorsMiddlewareTools(server) {
    server.tool("cors_generate_middleware", "Generate CORS middleware", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "cors_generate_middleware", directory, instructions: "Generate CORS middleware in the specified directory." }, null, 2) }] };
    });
    server.tool("cors_analyze_config", "Analyze CORS configuration", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "cors_analyze_config", directory, instructions: "Analyze CORS configuration in the specified directory." }, null, 2) }] };
    });
    server.tool("cors_generate_policy", "Generate CORS policy documentation (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "cors_generate_policy", directory, instructions: "Generate CORS policy documentation in the specified directory." }, null, 2) }] };
    });
}
