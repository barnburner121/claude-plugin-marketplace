...
export function registerMultiTenantTools(server) {
    server.tool("tenant_design_schema", "Design multi-tenant database schema with isolation strategy selection", {
    ...
    server.tool("tenant_generate_middleware", "Generate tenant resolution middleware with context propagation", {
    ...
    server.tool("tenant_isolation_setup", "Pro: Set up complete tenant data isolation with RLS, connection pooling, and resource limits", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The tenant_isolation_setup tool requires a Pro subscription. Please upgrade to access tenant isolation setup." }, null, 2),
                    },
                ],
            };
        }
