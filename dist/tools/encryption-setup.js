...
export function registerEncryptionSetupTools(server) {
    server.tool("encrypt_setup_atrest", "Generate encryption at rest setup", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "encrypt_setup_atrest", directory, instructions: "Generate encryption at rest setup in the specified directory." }, null, 2) }] };
    });
    server.tool("encrypt_setup_intransit", "Generate encryption in transit", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "encrypt_setup_intransit", directory, instructions: "Generate encryption in transit in the specified directory." }, null, 2) }] };
    });
    server.tool("encrypt_key_management", "Generate key management system (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "encrypt_key_management", directory, instructions: "Generate key management system in the specified directory." }, null, 2) }] };
    });
}
