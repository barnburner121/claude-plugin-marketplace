...
export function registerSearchIndexTools(server) {
    server.tool("searchidx_setup", "Generate search indexing setup", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "searchidx_setup", directory, instructions: "Generate search indexing setup in the specified directory." }, null, 2) }] };
    });
    server.tool("searchidx_add_autocomplete", "Add search autocomplete", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "searchidx_add_autocomplete", directory, instructions: "Add search autocomplete in the specified directory." }, null, 2) }] };
    });
    server.tool("searchidx_optimize", "Optimize search relevance (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "searchidx_optimize", directory, instructions: "Optimize search relevance in the specified directory." }, null, 2) }] };
    });
}
