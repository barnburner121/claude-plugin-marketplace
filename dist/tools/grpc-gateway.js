...
export function registerGrpcGatewayTools(server) {
    server.tool("grpcgw_generate", "Generate gRPC-Gateway HTTP bridge", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "grpcgw_generate", directory, instructions: "Generate gRPC-Gateway HTTP bridge in the specified directory." }, null, 2) }] };
    });
    server.tool("grpcgw_add_swagger", "Add Swagger from gRPC definitions", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "grpcgw_add_swagger", directory, instructions: "Add Swagger from gRPC definitions in the specified directory." }, null, 2) }] };
    });
    server.tool("grpcgw_optimize", "Optimize gateway performance (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "grpcgw_optimize", directory, instructions: "Optimize gateway performance in the specified directory." }, null, 2) }] };
    });
}
