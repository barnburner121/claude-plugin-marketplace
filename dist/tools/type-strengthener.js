...
export function registerTypeStrengthenerTools(server) {
    server.tool("type_find_any", "Find all usages of 'any' type in TypeScript code and suggest proper types", {
    ...
    server.tool("type_suggest_generics", "Find functions that could benefit from generic types for better type safety", {
        directory: z.string().describe("Project directory to scan"),
    ...
    server.tool("type_improve_interfaces", "Audit and improve TypeScript interfaces and type definitions (Pro feature)", {
        directory: z.string().describe("Project directory to scan"),
        api_key: z.string().optional(),
    }, async ({ directory, api_key }) => {
        const { allowed, tier } = checkRateLimit(api_key);
