...
export function registerRequestSigningTools(server) {
    server.tool("reqsign_generate_hmac", "Generate HMAC request signing", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "reqsign_generate_hmac", directory, instructions: "Generate HMAC request signing in the specified directory." }, null, 2) }] };
    });
    server.tool("reqsign_generate_aws", "Generate AWS Signature V4", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "reqsign_generate_aws", directory, instructions: "Generate AWS Signature V4 in the specified directory." }, null, 2) }] };
    });
    server.tool("reqsign_add_verification", "Generate signature verification (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "reqsign_add_verification", directory, instructions: "Generate signature verification in the specified directory." }, null, 2) }] };
    });
}
