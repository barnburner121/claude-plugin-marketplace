...
export function registerPciScannerTools(server) {
    server.tool("pci_scan_codebase", "Scan codebase for PCI-DSS issues", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "pci_scan_codebase", directory, instructions: "Scan codebase for PCI-DSS issues in the specified directory." }, null, 2) }] };
    });
    server.tool("pci_generate_checklist", "Generate PCI compliance checklist", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "pci_generate_checklist", directory, instructions: "Generate PCI compliance checklist in the specified directory." }, null, 2) }] };
    });
    server.tool("pci_generate_report", "Generate PCI compliance report (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "pci_generate_report", directory, instructions: "Generate PCI compliance report in the specified directory." }, null, 2) }] };
    });
}
