...
export function registerCacheArchitectTools(server) {
    server.tool("cache_design_strategy", "Design a caching strategy with TTL policies, cache layers, and eviction rules", {
    ...
    server.tool("cache_generate_redis", "Generate Redis caching implementation with connection pooling and serialization", {
    ...
    server.tool("cache_invalidation_setup", "Pro: Set up cache invalidation with event-driven purging and tag-based invalidation", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The cache_invalidation_setup tool requires a Pro subscription. Please upgrade to access cache invalidation setup." }, null, 2),
                    },
                ],
            };
        }
