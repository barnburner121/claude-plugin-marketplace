...
export function registerApiForgeTools(server) {
    // Tool 1: Generate API test suite from OpenAPI spec or route files
    server.tool("api_generate_tests", "Generate comprehensive API test suite from OpenAPI spec, route definitions, or endpoint patterns", {
        source: z
            .string()
            .describe("Path to OpenAPI spec file, routes file, or directory to scan"),
        framework: z
    ...
    server.tool("api_create_mock", "Generate a mock API server that returns realistic fake data matching your API schema", {
        spec_path: z
            .string()
    ...
    server.tool("api_contract_test", "Verify API contract compatibility between a provider spec and consumer expectations (Pro feature)", {
        provider_spec: z
            .string()
    ...
    server.tool("api_benchmark", "Create a performance benchmark configuration for API endpoints", {
        endpoints: z
            .array(z.object({
