...
export function registerFlakyTestFinderTools(server) {
    server.tool("flaky_detect_patterns", "Analyze test code to detect common flaky test patterns and anti-patterns", {
            ...
            content: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        instructions: "Analyze the provided test code for common flaky test patterns: race conditions, shared mutable state, time-dependent assertions, network dependency without mocking, non-deterministic data, improper async handling, test order dependency, hardcoded timeouts, and missing test isolation. Report each detected pattern with its location, severity, and explanation of why it causes flakiness.",
                    }, null, 2),
                },
            ],
        };
    });
    server.tool("flaky_diagnose_root_cause", "Diagnose the root cause of a specific flaky test from code and failure logs", {
            ...
            content: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        instructions: "Analyze the flaky test code alongside its failure logs to identify the root cause of intermittent failures. Examine timing issues, external dependencies, state management, assertion precision, and environmental factors. Cross-reference the test with the source code under test to find race conditions or non-deterministic behavior. Provide a clear diagnosis with evidence from the failure logs.",
                    }, null, 2),
                },
            ],
        };
    });
    server.tool("flaky_fix_suggestions", "Generate specific code fixes to stabilize flaky tests (Pro)", {
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            instructions: "This tool requires a Pro subscription. Upgrade to Pro to get specific code fixes and refactored test implementations to stabilize your flaky tests.",
                        }, null, 2),
                    },
                ],
            };
