...
export function registerMobileCiTools(server) {
    server.tool("mobileci_setup_fastlane", "Set up Fastlane for mobile CI/CD", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "mobileci_setup_fastlane", directory, instructions: "Set up Fastlane for mobile CI/CD in the specified directory." }, null, 2) }] };
    });
    server.tool("mobileci_add_signing", "Add code signing configuration", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "mobileci_add_signing", directory, instructions: "Add code signing configuration in the specified directory." }, null, 2) }] };
    });
    server.tool("mobileci_add_deployment", "Generate app store deployment pipeline (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "mobileci_add_deployment", directory, instructions: "Generate app store deployment pipeline in the specified directory." }, null, 2) }] };
    });
}
