...
export function registerEtlBuilderTools(server) {
    server.tool("etl_design_pipeline", "Design an ETL/ELT pipeline with source, transform, and load stages", {
    ...
    server.tool("etl_generate_transforms", "Generate data transformation functions with validation, mapping, and enrichment", {
    ...
    server.tool("etl_schedule_setup", "Pro: Set up ETL pipeline scheduling with cron jobs, dependency management, and monitoring", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The etl_schedule_setup tool requires a Pro subscription. Please upgrade to access ETL scheduling setup." }, null, 2),
                    },
                ],
            };
        }
