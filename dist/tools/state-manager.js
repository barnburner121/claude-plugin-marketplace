...
export function registerStateManagerTools(server) {
    server.tool("state_setup_store", "Set up a state management store with typed state, actions, and selectors", {
    ...
    server.tool("state_generate_slices", "Generate state slices with reducers, actions, and async thunks", {
        library: z.enum(["redux-toolkit", "zustand", "jotai", "mobx", "pinia", "vuex"]).describe("State management library"),
    ...
    server.tool("state_add_persistence", "Add state persistence with storage adapters, migration, and rehydration (Pro)", {
        library: z.enum(["redux-toolkit", "zustand", "jotai", "mobx", "pinia", "vuex"]).describe("State management library"),
