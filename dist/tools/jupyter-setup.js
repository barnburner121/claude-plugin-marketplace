...
export function registerJupyterSetupTools(server) {
    server.tool("jupyter_init_project", "Initialize Jupyter notebook project with kernels", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "jupyter_init_project", directory, instructions: "Initialize Jupyter notebook project with kernels in the specified directory." }, null, 2) }] };
    });
    server.tool("jupyter_add_extensions", "Add Jupyter extensions and widgets", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "jupyter_add_extensions", directory, instructions: "Add Jupyter extensions and widgets in the specified directory." }, null, 2) }] };
    });
    server.tool("jupyter_configure_kernels", "Configure custom kernels and environments (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "jupyter_configure_kernels", directory, instructions: "Configure custom kernels and environments in the specified directory." }, null, 2) }] };
    });
}
