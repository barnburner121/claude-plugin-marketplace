...
export function registerFeatureFlagTools(server) {
    server.tool("flag_setup_system", "Set up a feature flag system with storage, evaluation engine, and admin API", {
    ...
    server.tool("flag_generate_checks", "Generate feature flag check utilities and middleware for gating features", {
    ...
    server.tool("flag_ab_test_setup", "Pro: Set up A/B testing with variant assignment, metrics tracking, and statistical analysis", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The flag_ab_test_setup tool requires a Pro subscription. Please upgrade to access A/B testing setup." }, null, 2),
                    },
                ],
            };
        }
