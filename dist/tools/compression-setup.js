...
export function registerCompressionSetupTools(server) {
    server.tool("compress_setup_gzip", "Set up gzip/brotli compression", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "compress_setup_gzip", directory, instructions: "Set up gzip/brotli compression in the specified directory." }, null, 2) }] };
    });
    server.tool("compress_add_static", "Add static asset compression", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "compress_add_static", directory, instructions: "Add static asset compression in the specified directory." }, null, 2) }] };
    });
    server.tool("compress_optimize", "Optimize compression settings (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "compress_optimize", directory, instructions: "Optimize compression settings in the specified directory." }, null, 2) }] };
    });
}
