...
export function registerDisasterRecoveryTools(server) {
    server.tool("dr_assess_risks", "Assess disaster recovery risks for infrastructure components", {
    ...
    server.tool("dr_generate_plan", "Generate a comprehensive disaster recovery plan document", {
    ...
    server.tool("dr_create_runbooks", "Create detailed DR runbooks with step-by-step recovery procedures (Pro)", {
        system_name: z.string().describe("Name of the system"),
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The dr_create_runbooks tool requires a Pro subscription. Please upgrade to access DR runbook creation features." }, null, 2),
                    },
                ],
            };
        }
