...
export function registerQueryOptimizerTools(server) {
    server.tool("query_analyze_slow", "Analyze slow database queries and identify performance bottlenecks", {
    ...
    server.tool("query_suggest_indexes", "Suggest database indexes based on query patterns and table structure", {
    ...
    server.tool("query_rewrite", "Pro: Rewrite SQL queries for optimal performance while preserving correctness", {
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The query_rewrite tool requires a Pro subscription. Please upgrade to access query rewriting." }, null, 2),
                    },
                ],
            };
        }
