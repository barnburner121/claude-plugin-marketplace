...
export function registerExperimentTrackerTools(server) {
    server.tool("exp_setup_tracking", "Set up experiment tracking with MLflow", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "exp_setup_tracking", directory, instructions: "Set up experiment tracking with MLflow in the specified directory." }, null, 2) }] };
    });
    server.tool("exp_add_metrics", "Add custom metric tracking", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        if (!allowed)
            return rateLimitError(tier);
        return { content: [{ type: "text", text: JSON.stringify({ action: "exp_add_metrics", directory, instructions: "Add custom metric tracking in the specified directory." }, null, 2) }] };
    });
    server.tool("exp_generate_reports", "Generate experiment comparison reports (Pro feature)", { directory: z.string().describe("Project directory"), api_key: z.string().optional() }, async ({ directory, api_key }) => {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ action: "exp_generate_reports", directory, instructions: "Generate experiment comparison reports in the specified directory." }, null, 2) }] };
    });
}
