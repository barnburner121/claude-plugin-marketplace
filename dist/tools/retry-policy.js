...
export function registerRetryPolicyTools(server) {
    server.tool("retry_generate_middleware", "Generate retry middleware with configurable policies for HTTP clients", {
    ...
    server.tool("retry_configure_backoff", "Configure backoff strategies for retry policies", {
    ...
    server.tool("retry_add_circuit_breaker", "Combine retry policies with circuit breaker for resilient communication (Pro)", {
        service_name: z.string().describe("Name of the service"),
        language: z.string().describe("Programming language"),
        ...
        if (tier !== "pro") {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ action: "upgrade_required", instructions: "The retry_add_circuit_breaker tool requires a Pro subscription. Please upgrade to access combined retry and circuit breaker features." }, null, 2),
                    },
                ],
            };
        }
